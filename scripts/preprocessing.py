...
class ReviewPreprocessor:
    ...
    def remove_duplicates(self):
        """Remove duplicate reviews based on meaningful fields"""
        print("\n[2/7] Removing duplicates...")

        before_count = len(self.df)

        # Only use columns likely to duplicate across banks
        duplicate_columns = [
         'review_text',
         'user_name',
         'rating',
        ...
        duplicate_columns = [col for col in duplicate_columns if col in self.df.columns]

        self.df = self.df.drop_duplicates(subset=duplicate_columns, keep='first')

        removed = before_count - len(self.df)
        self.stats['duplicate_removed'] = removed
